...
struct ContentView: View {
    ...
    private var theme_colors: (bgColor: Color, cardBgColor: Color, fontColor: Color) {
        GameColors.theme_colors(game.emoji_theme)
    }
    ...
    let columns: [GridItem] = Array(repeating: .init(.adaptive(minimum: 90, maximum: 120), spacing: 5), count: 4);
    
    var body: some View {
        VStack {
            Text("Theme \(game.emoji_theme)").font(.largeTitle).fontWeight(/*@START_MENU_TOKEN@*/.bold/*@END_MENU_TOKEN@*/).padding(.top)
            HStack{
            ...
            if game.cards.filter({!$0.isMatched}).count == 0 {
                ...
                LazyVGrid(columns: columns, alignment: .center, spacing: 5 , content: {
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        return Group {
            ...
            //            ContentView().previewLayout(.fixed(width: 814, height: 414))
            ContentView().previewLayout(.fixed(width: 845, height: 428))
        }
    }
}

struct GameColors {
    static func theme_colors(_ emoji_theme: String) -> (bgColor: Color, cardBgColor: Color, fontColor: Color) {
        switch emoji_theme {
        case "Animals":
            return (
